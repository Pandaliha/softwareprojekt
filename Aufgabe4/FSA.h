...
//um alle Zustände, die mit epsilkon erreichbar sind zu finden, erst alle Zustände durchlaufen, dann alle Übergänge
void FSA::closure()
{
    for(int i = 0; i < current.size(); i++)
    {
        ...
        {
            if(ts[j].trigger(current[i]))
            {
                if(find(current.begin(),current.end(),ts[j].toState()) == current.end())
                {
                    current.push_back(ts[j].toState());
                }
            }
        }
    }
